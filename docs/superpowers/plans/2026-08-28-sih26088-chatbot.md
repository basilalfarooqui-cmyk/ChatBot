...
Run: `npx tsc --noEmit`
Expected: no errors (note: `language-select` and `(main)` routes don't exist yet — that's fine, `Stack.Screen name` is a plain string, not type-checked against the filesystem until `expo-router`'s typed routes generate after those files exist in Task 7/12).
